...
class Cabbage(Dataset):
    def __init__(self, split='train', data_root='trainval', test_area=5, voxel_size=0.04, voxel_max=None,
                 transform=None, shuffle_index=False, loop=1, use_edge=None, edge_num=None, args=None):
        super().__init__()
        ...
        self.train_list = np.loadtxt(args.train_list, dtype=str)
        self.test_list = np.loadtxt(args.test_list, dtype=str)
        if split == 'train':
            self.data_list = [item for item in self.train_list if args.crop_class in item]
        else:
            self.data_list = [item for item in self.test_list if args.crop_class in item]
    ...
    def __getitem__(self, idx):
        ...
        _, __, cls, fn = file_name.split("/")
        data_name = cls + "_" + fn
        # data = read_ply2np(self.data_list[data_idx])
        data = SA.attach("shm://{}".format(data_name)).copy()

        if self.use_edge:
            edge = SA.attach("shm://{}".format(data_name + "_edge")).copy()
            ...
            if data.shape[1] == 7:
                ...
                coord, feat, label = (data[:, 0:3] - np.mean(data[:, 0:3], axis=0)) *10, np.zeros_like(data[:, :3]), data[:, 3]
