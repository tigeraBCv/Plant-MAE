...
def get_edge(file_name):
    ...
    if not os.path.exists("/dev/shm/{}".format(file_name + "_edge")):
        ...
        if data.shape[0] > 100000:
            ran_sample = np.random.choice(len(data), 100000, replace=False)
...
class S3DIS(Dataset):
    def __init__(self, split='train', data_root='trainval', test_area=5, voxel_size=0.04, voxel_max=None, transform=None, shuffle_index=False, loop=1,edge_num=2000):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        coord, feat, label = data[:, 0:3]-np.mean(data[:, 0:3], axis=0), data[:, 3:6], data[:, 6]
        coord, feat, label = data_prepare(coord, feat, label, self.split, self.voxel_size, self.voxel_max, self.transform, self.shuffle_index,self.edge_num)
