...
def collate_fn_limit(batch, max_batch_points, logger):
    ...
    if logger is not None and k < len(batch):
        ...
        s = sum([x.shape[0] for x in coord])
        s_now = sum([x.shape[0] for x in coord[:k]])
        print("batch_size shortened from {} to {}, points from {} to {}".format(len(batch), k, s, s_now))
    else:
        s = sum([x.shape[0] for x in coord])
        # print("batch_size points from {}".format(s))
    return torch.cat(coord[:k]), torch.cat(feat[:k]), torch.cat(label[:k]), torch.IntTensor(offset[:k])
    # return torch.cat(coord), torch.cat(feat), torch.cat(label), torch.IntTensor(offset)
...
def setdiff2d_idx(arr1, arr2):
    ...
    # arr1 = set(map(tuple, arr1))
    idx = [tuple(x) not in delta for x in arr1]
    return idx

def data_prepare_edge(coord, feat, label, split='train', voxel_size=0.04, voxel_max=None, transform=None, shuffle_index=False, edge_num=None):
    if transform:
        # coord, feat, label = transform(coord, feat, label)
        coord, feat = transform(coord, feat)
        data = np.concatenate([coord, feat, label.reshape(-1, 1)], axis=1)
    else:
        data = np.concatenate([coord, feat, label.reshape(-1, 1)], axis=1)
    ...
    coord, feat, label = np.concatenate([coord, data[-edge_num:, :3]], axis=0), \
                         np.concatenate([feat, data[-edge_num:, 3:6]], axis=0), \
                         np.concatenate([label, data[-edge_num:, 6]], axis=0)

    ## Deduplication
    # idx = np.asarray(setdiff2d_idx(coord[:, :3], coord[-edge_num:, :3]))
    # idx[-edge_num:] = True
    # coord, feat, label = coord[idx], feat[idx], label[idx]


    # # Deduplication
    # coord_unique, uniq_idx = np.unique(coord, axis=0, return_index=True)
    # coord, feat, label = coord[uniq_idx], feat[uniq_idx], label[uniq_idx]
