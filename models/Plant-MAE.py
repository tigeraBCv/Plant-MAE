...
class H_Encoder(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.concat_xyz = config.concat_xyz
        ...
        for i in range(len(self.encoder_dims)):
            if i == 0:
                ...
                self.token_embed.append(KPConvSimpleBlock_second(in_channels=3 if not self.concat_xyz else 6,out_channels=self.encoder_dims[i],prev_grid_size=0.04,max_num_neighbors=self.max_num_neighbors))
        ...
        for i in range(len(self.encoder_depths)):
            self.encoder_blocks.append(Encoder_Block(
                            ...
                            neighbours=config.local_number[i]
                        ))
    ...
    def forward(self, xyz,neighborhoods, centers, idxs, eval=False):
        ...
        for i in range(len(centers)):
            ...
            #feats, xyz, batch, neighbor_idx
            else:
                # x_vis_L = []
                # x_vis_L.append(x_vis)
                # b, g1, _ = x_vis.shape
                # b, g2, k2, _ = neighborhoods[i].shape
                # #x_vis_neighborhoods = x_vis.reshape(b * g1, -1)[idxs[i], :].reshape(b, g2, k2, -1)
                # x_vis_neighborhoods = x_vis.reshape(b * g1, -1)[idxs[i], :].reshape(b, g2, k2, -1)
                # group_input_tokens = self.token_embed[i](xyz=x_vis_neighborhoods.reshape(b,g2*k2,-1),centors=centers[i],idx=idxs[i])
                b, g1, _ = x_vis.shape
                b, g2, k2, _ = neighborhoods[i].shape
                x_vis_neighborhoods = x_vis.reshape(b * g1, -1)[idxs[i], :].reshape(b, g2, k2, -1)
            ...
            x_vis = self.encoder_blocks[i](x_vis,pos,masked_center)
...
class KPConvSimpleBlock_second(nn.Module):
    def __init__(self, in_channels, out_channels,prev_grid_size=0.04,max_num_neighbors=16, sigma=1.0, negative_slope=0.2, bn_momentum=0.02):
        super().__init__()
...
class Point_M2AE_ST(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[Point_M2AE_add_kp]', logger ='Point_M2AE_st')
        self.config = config
        # tokenizers
    ...
    def forward(self, pts, eval=False, **kwargs):
        ...
        for i in range(len(self.group_dividers)):
            if i == 0:
                ...
                # feat = np.zeros((len(center)*len(center[0,:,:]), 3)).reshape(B,N,C)
        ...
        if eval:
            # for linear svm
            x_vis_list, mask_vis_list, _ = self.h_encoder(pts,neighborhoods, centers, idxs,eval=True)
            x_vis = x_vis_list[-1]
            return x_vis.mean(1) + x_vis.max(1)[0]
        else:
            x_vis_list, mask_vis_list, masks = self.h_encoder(pts,neighborhoods, centers, idxs)
            # xyz, neighborhoods, centers, idxs, feats, eval = False
        # hierarchical decoder
        centers.reverse()
        ...
        for i in range(len(self.decoder_dims)):
            ...
            x_full = self.h_decoder[i](x_full, pos_full,center)
