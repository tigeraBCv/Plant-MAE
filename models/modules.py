...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, xyz):
        ...
        idx = idx + idx_base
        idx = idx.view(-1)
        # has_duplicates = torch.unique(idx).shape[0] != idx.shape[0]
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        #     embed_channels=dim, groups=4, qkv_bias=qkv_bias, attn_drop_rate=attn_drop, pe_multiplier=True,
        #     pe_bias=True)
        # self.attn = Attention_v2(
        #     dim, num_heads=num_heads, qkv_bias=True, qk_scale=qk_scale,pe_multiplier=True, attn_drop=attn_drop, proj_drop=drop)
        # self.attn = Attention_v3(dim,dim)
    def forward(self, x, mask=None):
        x = x + self.drop_path(self.attn(self.norm1(x), mask))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x
    # def forward(self, feat, xyz):  #attention xiu zheng
    #     feat = feat + self.drop_path(self.attn(self.norm1(feat), coord=xyz))
    #     x = feat + self.drop_path(self.mlp(self.norm2(feat)))
    #     return x
    # def forward(self, x, mask=None,coord=None):
    #     x = x + self.drop_path(self.attn(self.norm1(x), mask,coord=coord))
    #     x = x + self.drop_path(self.mlp(self.norm2(x)))
    #     return x
    # def forward(self, feat, xyz): #cuo wu
    #     x = feat + self.drop_path(self.attn(self.norm1(feat),xyz))
    #     x = feat + self.drop_path(self.mlp(self.norm2(feat)))
    #     return x
...
class Encoder_Block(nn.Module):
    ...
    def forward(self, x, pos, vis_mask):
        ...
        return x
    # def forward(self, x, pos, vis_mask, coord):
    #     for _, block in enumerate(self.blocks):
    #         x = block(feat=x + pos, xyz=coord, mask=vis_mask)
    #     return x
    # def forward(self, x, pos,coord):
    #     for _, block in enumerate(self.blocks):
    #         x = block(feat=x + pos,xyz=coord)
    #     return x
    # def forward(self, x, pos, xyz):  #attention xiu zheng
    #     for _, block in enumerate(self.blocks):
    #         x = block(feat=x + pos, xyz=xyz)
    #     return x
...
class GroupedVectorAttention(nn.Module):
    def __init__(self, embed_channels, groups=8, attn_drop_rate=0.0, qkv_bias=True, pe_multiplier=False, pe_bias=True,neighbours=16):
        ...
        self.weight_encoding = nn.Sequential(
            nn.Linear(embed_channels, embed_channels//self.neighbours),
            PointBatchNorm(embed_channels//self.neighbours),
            nn.ReLU(inplace=True),
            nn.Linear(embed_channels//self.neighbours, embed_channels//self.neighbours),
        )
        ...
        self.a = nn.Parameter(torch.zeros(size=(self.neighbours, 1)), requires_grad=True)
        self.b = nn.Parameter(torch.zeros(size=(self.neighbours, embed_channels)), requires_grad=True)
        self.d = nn.Parameter(torch.zeros(size=(self.neighbours, embed_channels)), requires_grad=True)
        ...
        self.knn = KNN(k=self.neighbours, transpose_mode=True)
    ...
    def forward(self, feat, coord):
        ...
        dist, reference_index = self.knn(coord, coord)
        B,N,C = feat.shape
        key = pointops.grouping(reference_index.reshape(B*N,-1), key.reshape(B*N,-1), coord.reshape(B*N,-1), with_xyz=True)
        value = pointops.grouping(reference_index.reshape(B*N,-1), value.reshape(B*N,-1), coord.reshape(B*N,-1), with_xyz=False)
        pos, key = key[:, :, 0:3], key[:, :, 3:]
        relation_qk = key - query.reshape(B*N,-1).unsqueeze(1)
        ...
        b_feat = self.d * all_feat
        value = value+b_feat
        # ================================================== Weight ==================================================
        weight = self.weight_encoding(relation_qk)
        ###添加新的特征
        weight = self.attn_drop(self.softmax(weight))
        # mask = torch.sign(reference_index.reshape(B*N,-1) + 1)
        # weight = torch.einsum("n s g, n s -> n s g", weight, mask)
        value = einops.rearrange(value, "n ns (g i) -> n ns g i", g=self.embed_channels//self.neighbours)
...
class Attention_v3(nn.Module):
    def __init__(self, embed_channels, out_planes, share_planes=8, nsample=16):
        super().__init__()
        ...
        self.linear_q = nn.Linear(embed_channels, embed_channels)
        self.linear_k = nn.Linear(embed_channels, embed_channels)
        ...
        self.linear_uni_spatial = nn.Sequential(nn.Linear(embed_channels, 1), nn.BatchNorm1d(1), nn.ReLU(inplace=True))
    ...
    def forward(self, feat, coord) -> torch.Tensor:
        B, N, C = feat.shape
        ...
        dist, reference_index = self.knn(p, p)
        x_k = pointops.grouping(reference_index.reshape(B * N, -1), x_k.reshape(B * N, -1), coord.reshape(B * N, -1),
                                with_xyz=True)
        x_v_j = pointops.grouping(reference_index.reshape(B * N, -1), x_v.reshape(B * N, -1),
                                  coord.reshape(B * N, -1), with_xyz=False)
        e, x_k = x_k[:, :, 0:3], x_k[:, :, 3:]
