...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,neighbours =16):
        super().__init__()
        ...
        self.attn = GroupedVectorAttention(
            ...
            pe_bias=True,neighbours=neighbours)

    def forward(self, feat, xyz):  #attention xiu zheng
        x = feat + self.drop_path(self.attn(self.norm1(feat), coord=xyz))
        x = feat + self.drop_path(self.mlp(self.norm2(x)))
        return x
    # def forward(self, feat, xyz):  #attention xiu zheng
    #     x = feat + self.drop_path(self.attn(self.norm1(feat), coord=xyz))
    ...
    # def forward(self, feat, xyz):  #attention xiu zheng 0704
    #     identity = feat
    #     feat=self.attn(self.norm1(feat), coord=xyz)
    #     feat=self.mlp(self.norm2(feat))
...
class Encoder_Block(nn.Module):
    def __init__(self, embed_dim=768, depth=4, num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., neighbours=16):
        super().__init__()
...
class GroupedVectorAttention(nn.Module):
    def __init__(self, embed_channels, groups=8, attn_drop_rate=0.0, qkv_bias=True, pe_multiplier=True, pe_bias=True,neighbours=16):
        ...
        self.linear_add = nn.Sequential(nn.Linear(3, 3), nn.BatchNorm1d(3), nn.ReLU(inplace=True),
                                        nn.Linear(3, embed_channels))
        self.linear_p = nn.Sequential(nn.Linear(3, 3), nn.BatchNorm1d(3), nn.ReLU(inplace=True), nn.Linear(3, embed_channels))
    ...
    def forward(self, feat, coord):
        ...
        for i, layer in enumerate(self.linear_p):
            pos = layer(pos.transpose(1, 2).contiguous()).transpose(1, 2).contiguous() if i == 1 else layer(pos)

        # b_feat = self.d * all_feat
        b_feat = pos + self.d * all_feat
        value = value + b_feat
        # ================================================== Weight ==================================================
